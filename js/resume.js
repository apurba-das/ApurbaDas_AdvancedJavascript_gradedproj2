...
function checkAndDisableNavButtons(searchResult) {
	...
	if(currResultIdx === allIds.length - 1) {
		...
		document.getElementById('next-btn').style.visibility='none';
		}
	else if (currResultIdx === 0){
		document.getElementById('prev-btn').style.display='none';
		document.getElementById('prev-btn').style.visibility='none';
	}
	else{
	...
	if(searchResults.length === 0) {
		listDiv.innerHTML = `
			<div style="border: 1px solid black; border-radius: 10px; padding: 20px; margin-left: 20%; margin-right: 20%">
				<img width='80' height='80' src='sorry.jpg' alt='No such results found'/>
				<span style="text-align: center; position: absolute; top: 100px;"> No such results found</span>
			</div>
		`;
	}
